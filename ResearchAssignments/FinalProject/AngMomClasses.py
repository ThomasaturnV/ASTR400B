...
class HostGalaxy_AngMomEvolution:
    ...
    def PlotAngMom(self, iteration):
        ...
        # Plot Formatting #
        plt.xlabel('Radial Shell (kpc)', fontsize = 20)
        plt.ylabel(r'Angular Momentum $(10^{10} kpc \, \dot{M}_{\odot} \, \mathrm{km/s})$', fontsize = 20)
        plt.title(f'Angular Momentum Distribution of {self.GalaxyName} at {self.Time[(iteration - 1)]} Gyr', fontsize = 30) 
    ...
    def GifGeneration(self):
        ...
        # Frames List (To save each frame of the gif) #
        Frames = [Image.open(self.PlotAngMom(t + 1)) for t in range(len(self.Time))]
...
class SatelliteGalaxy_AngMomEvolution:
    ...
    def PlotAngMom(self):
        ...
        plt.ylabel(r'Orbital Angular Momentum $(10^{10} kpc \, \dot{M}_{\odot} \, \mathrm{km/s})$', fontsize = 15)
        plt.title(f'Orbital Angular Momentum Evolution of {self.SatGalaxyName} Orbiting {self.HostGalaxyName}', fontsize = 25)
...
def MAIN():
    ...
    #M33.OrbitalAngularMomentumEvolution(0, 800, 80)
    ...
    # Results
    M31.AngularMomentumEvolution(0, 800, 5)
    M31.ReadAngMomFile('M31_DMAngularMomentum.txt')
    M31.GifGeneration()

    M33.OrbitalAngularMomentumEvolution(0, 800, 5)
    M33.ReadAngMomFile('M33_OrbitalAngularMomentum.txt')
    M33.PlotAngMom()
### END MAIN
