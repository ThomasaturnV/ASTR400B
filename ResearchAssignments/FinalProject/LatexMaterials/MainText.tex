...
\begin{abstract}
Dynamical friction can be seen in galactic interactions where the gravitational scattering of particles causes a redistribution of material, introducing new perturbing forces. Dynamical friction plays an essential role in galaxy evolution as it can affect important evolution parameters such as merger timescales, merger classifications, and star formation. Within this study the Milky Way (MW), M31, M33 merging simulation developed by \citep{vandermarel2012} will be utilized to monitor the effects caused by dynamical friction within the merger of M31 and M33. This study addresses the evolution of angular momentum as a result of dynamical friction, specifically investigating the change in distribution of angular momentum as a result of the dark matter (DM) wake induced by M33 on M31. It was found that the orbital angular momentum of M33 decayed over the course of the simulation, decreasing by upwards of a factor of 100. Additionally, the DM wake induced by M33 was detectable and represented a considerable amount of the total angular momentum of M31. These results are essential to better quantifying the effects of dynamical friction in order to model more complex galaxy interactions and build more precise models to predict other galactic events. 
\end{abstract}
...
Within this study, I implemented the GADGET-3 MW, M31, and M33 merging simulation developed by \cite{vandermarel2012}. This simulation was done as a collisionless N-body model, including stars and dark matter in both a high-resolution and low-resolution capacity. An N-body simulation is one where the system is broken into N interacting particles under the influence of gravity to model an extended system. The initial conditions for this simulation (as modeled by snapshot 0) were determined through observational constraints. The authors constructed the stellar disks by implementing an exponential profile of a specific scale length, and the dark-matter distribution was modeled as a Hernquist profile. 

\vspace{10pt}

To accomplish this study, I will need to determine the dynamical friction effects on angular momentum due to the DM wake-induced transfer. First, I found the center-of-mass frame for M31 at each time using the DM particles for the low-resolution model. Then, for this center-of-mass reference frame, I updated all the position vectors for M31 particles and M33 particles. Next, to further refine the effect of the DM wake, I implemented an angular momentum shell method to reveal the radial distribution. Here, I computed the angular momentum contribution of the DM particles in a series of radii from the center of mass position up to a limiting radius. This limiting radius will be a function of scale length and will act to de-bias very far out particles that will dominate the angular momentum contribution, effectively adding random noise. By computing the angular momentum contribution in each shell, the shell containing a majority of the DM wake should see some change as a result of the M33 interaction. Additionally, using the M31 center of mass point, I determined the angular momentum contribution of the M33 particles to see the M33 system's angular momentum decay as the orbit slows and falls toward the M31 galaxy. Again, there will need to be a limiting radius for M33 as well to counteract the particles very far from the M33 center. Finally, I continued this methodology in time to see how the system develops, tracking the contribution of the DM wake as a function of time and analyzing how the angular momentum is shifted throughout the system. Figure 2 shows a flowchart outlining my methodology. 

\vspace{10pt}

To implement this study, I developed a code in python to do these calculations. Specifically when taking the displacement vector for the new reference frame the equation for such is outlined as: $ \vec{r_{com_i}} = \vec{r_i}-\vec{R_{com_i}} $, where $ \vec{r_{com_i}} $ is the position vector in the center of mass frame for any "ith" particle, $ \vec{r_i} $ is the position vector for the "ith" particle, and $ \vec{R_{com_i}}$ is the position for the center of mass for M31. The same would apply for the velocity vectors as well. The contribution of angular momentum of a specific particle will be: $ \sum_{i=1}^{N} \vec{L_i} = \sum_{i=1}^{N} m_i (\vec{r_{com_i}} \times \vec{v_{com_i}}) $. The limiting radius for each galaxy would be an interger multiple of the scale length of the Hernquist profile for the dark matter distribution \citep{hernquist1990}.

\scalebox{0.8}{
\begin{tikzpicture}[node distance=1.5cm and 0.5cm, every node/.style={text width=10cm, align=center, font=\large}]
        
        \node (start) [startstop] {1:Acquire center of mass position and velocity reference frames for DM particles of M31.};
        ...
        \node (step2) [process, below of=step1] {3:Determine the angular momentum contribution of every M31 particle utilizing the shell method.};
        \node (step3) [process, below of=step2] {4:Use the M33 particles to determine the angular momentum contribution of the M33 system.};
...
The plots generated by this code are twofold: One plot will be highlighting the orbital decay of the M33 system, and another will highlight the effect of the DM wake on the angular momentum of M31. The first plot is in the form of a GIF, showing the time evolution of the M31 system. The other shows the representation in time on the x-axis, and the M33 orbital angular momentum is on the y-axis. By showing this angular momentum evolution in time, the effects of dynamical friction can be visualized and characterized. 
...
After analyzing the resulting interaction within the simulation, the results outlined in Figure 3 found the presence of an angular momentum overabundance near the radial position of M33. Figure 3 shows an example of one frame of the GIF generated by the program, showing a higher distribution of angular momentum near the location of M33. The entire GIF is available in my GitHub repository linked \href{https://github.com/ThomasaturnV/ASTR400B/blob/main/ResearchAssignments/FinalProject/M31_AngularMomentumEvolution.gif}{here}. The overabundance of angular momentum begins to manifest around 4 Gyrs, afterwards, the trend is clear. Please note that the position of the peak does oscillate to some extent around M33's position but this is a reaction to the position of M33 oscillating as well.  This overabundance of angular momentum implies the presence of a DM wake, generating a higher density of angular momentum at a similar radial position as the satellite galaxy.

\noindent\includegraphics[width=1\linewidth]{M31_AngularMomentumEvolution-T6.857.png}
\noindent\textbf{Figure 3.} One frame of the angular momentum distribution GIF at a time of 6.857 Gyrs. The total angular momentum (y-axis) calculated within each shell of the radial distribution (x-axis) goes out to 15 scale lengths. The gray dotted line represented the radial position of M33. Due to the higher abundance of angular momentum located at M33, it is clear a DM wake is present and raising the total angular momentum within this specific region. The entire GIF is available \href{https://github.com/ThomasaturnV/ASTR400B/blob/main/ResearchAssignments/FinalProject/M31_AngularMomentumEvolution.gif}{here}.
...
\noindent\includegraphics[width=1\linewidth]{M33OrbitalAngMomEvolution.png}
...
\section{Conclusions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Dynamical friction was found to play an integral part in galactic interactions, where the scattering of particles causes a redistribution of material to perturb the system and induce an energy \& angular momentum transfer. This phenomena plays such a crucial role because it can affect important evolution parameters such as merger timescales, merger classifications, and star formation. By utilizing the MW, M31, M33 created by \citep{vandermarel2012}, the angular momentum evolution of the M31-M33 merger could be investigated. It was found that dynamical friction induced a DM wake around the radial position of M33, causing an overdensity of angular momentum to be clustered around its position. The resulting angular momentum transfer caused the orbital momentum of M31 to decay by upwards of a factor of 100. 

\vspace{10pt}

Through analysis of the angular momentum distribution of M31 (as shown in Figure 3) a DM wake can be clearly seen at a similar radial position of M33. This is an important finding for the field of galaxy evolution as the mass difference between M33 and M31 is greater than that of the Milky Way and the LMC by about a factor of 10. This means that the potential for a DM wake may be more common than previously thought as it is expected for systems of close or equivalent mass. However, due to the larger mass difference, it means that even a smaller satellite could be inducing significant angular momentum transfers, adding a necessity to consider other satellites within the galactic merging simulations. 

\vspace{10pt}

It was also found that the orbital angular momentum of the satellite decays rapidly through this dynamical friction process. Through analysis of the data, it can be seen that the very first pass (around 4 Gyrs) represents a majority of the angular momentum transfer process, and then it settles into a more steady decay. This is important as it could reshape the way researchers consider the angular momentum transfer, as it can be seen happening in "bursts" instead of a steady transfer. Due to the rapid in fall around 4 Gyrs, it is expected that a majority of the transfer would be anticipated at this point because there is the most interaction between the two systems' DM halos.  

\vspace{10pt}

To further this analysis, some future work is required. One way to better quantify the effect of the DM wake would be to add angular limits to the wake, determining the precise amount of angular momentum present in the trailing wake behind M33, as well as the physical extension of the wake. Presumably, the wake grows over time, increasing the rate the angular momentum is transferred. Additionally this study was limited to just the dark matter particles and treating M33 as a point mass. Where this not the case we can expect a more extended distribution to slow the effects of dynamical friction, as not all the mass is concentrated at one point. Utilizing an extended distribution for M33 and having more particle types considered could give a more accurate depiction for these effects, especially to see how the wake reacts to an extended distribution. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
I would like to thank Dr. Gurtina Besla and Himansh Rathore for their contributions to improving the project and the personal advice they have supplied throughout this project. 

\vspace{10pt}

This work made use of the following software packages: \texttt{astropy} \citep{astropy:2013, astropy:2018, astropy:2022}, \texttt{matplotlib} \citep{Hunter:2007}, \texttt{numpy} \citep{numpy}, \texttt{python} \citep{python}, and \texttt{scipy} \citep{2020SciPy-NMeth, scipy_14880408}.
